...
static int bt_vendor_qcom_op(bt_vendor_opcode_t opcode, void *param)
{
	...
	{
		bt_vendor_power_state_t power_state = *(uint8_t *)param;
		switch (power_state) {
		case BT_VND_PWR_OFF:
			bt_hardware_power(false);
			break;
		case BT_VND_PWR_ON:
			bt_hardware_power(true);
			break;
		}
		break;
	}
	...
	{
		int fd = bt_hardware_serial(true);
		if (fd > 0) {
	...
	{
		bt_vendor_lpm_wake_state_t lpm_wake_state = *(uint8_t *)param;
		bt_vendor_callbacks->lpm_set_state_cb(lpm_wake_state);
		break;
	}
#endif
